                                                                ...
                                                                DocumentListener
{
    ...
    public void undoableEditHappened (UndoableEditEvent e)
    {
        ...
        // JDK9 wraps this now and so this should be removed. AbstractDocument.DefaultDocumentEvent event = (AbstractDocument.DefaultDocumentEvent) e.getEdit ();

        int offsetChange = textComponent.getCaretPosition () - lastOffset;

        // JDK9 wraps the event so the type cannot be found, instead we use isSignificant.

        if (e.getEdit ().isSignificant ())
        // TODO Remove if (event.getType ().equals (DocumentEvent.EventType.CHANGE))
        {
