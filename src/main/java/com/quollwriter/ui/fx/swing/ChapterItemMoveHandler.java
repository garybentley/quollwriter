...
public class ChapterItemMoveHandler
{
    ...
    public void dragFinished ()
    {
        ...
        int v = this.editor.viewToModel2D (p);
        ...
        try
        {
            ...
            // TODO Needed? this.viewer.reloadTreeForObjectType (Chapter.OBJECT_TYPE);
    ...
    public void startDrag ()
    {
        ...
        Color c = SwingUIUtils.getDragIconColor ();

        String id = this.item.getObjectType () + SwingUIUtils.colorToHex (c);

        Image image = this.iconColumn.getIconProvider ().getIcon (this.item,
                                                                  Constants.ICON_COLUMN).getImage ();

        ImageIcon im = SwingUIUtils.getColoredIcon (image,
                                               c);
