...
public class EditorPanePrinter extends JPanel implements Pageable,
                                                         Printable
{
    ...
    class PagePanel extends JPanel
    {
        ...
        public PagePanel(int   pageStartY,
                         int   pageEndY,
                         Shape pageShape)
        {
    ...
    public EditorPanePrinter(JEditorPane pane,
                             Paper       paper,
                             Insets      margins)
    {
        initData (pane,
    ...
    public void initData (JEditorPane pane,
                          Paper       paper,
                          Insets      margins)
    {
    ...
    public void doPagesLayout ()
    {
        ...
        sourcePane.setSize (pageWidth - margins.top - margins.bottom,
                            Integer.MAX_VALUE);
        ...
        this.setPreferredSize (new Dimension ((pageWidth * 2) + 50,
                                              PAGE_SHIFT + (count * (pageHeight + PAGE_SHIFT))));
    }
    ...
    protected void calculatePageInfo ()
    {
        ...
        while ((startY + pageHeight - margins.top - margins.bottom) < sourcePane.getHeight ())
        {
            Shape pageShape = getPageShape (startY,
                                            ...
                                            pageHeight - margins.top - margins.bottom,
                                            sourcePane);
            pages.add (new PagePanel (startY,
                                      endPageY,
                                      pageShape));
            startY = endPageY;
        ...
        {
            add (pi);
            pi.setLocation (PAGE_SHIFT,
                            PAGE_SHIFT + (count * (pageHeight + PAGE_SHIFT)));
            count++;
        }
    }

    protected static Shape getAllocation (View        v,
                                          JEditorPane edit)
    {
    ...
    protected int getEndPageY (int startY)
    {
        ...
        {
            View v = getLeafViewAtPoint (new Point (x,
                                                    realY),
                                         rootView);

            if (v != null)
            {
                Rectangle alloc = EditorPanePrinter.getAllocation (v,
                                                                   sourcePane).getBounds ();
    ...
    protected View getLeafViewAtPoint (Point p,
                                       View  root)
    {
        return getLeafViewAtPoint (p,
                                   root,
                                   sourcePane);
    }

    public static View getLeafViewAtPoint (Point       p,
                                           View        root,
                                           JEditorPane sourcePane)
    {
        ...
        {
            int i = v.getViewIndex (pos,
                                    Position.Bias.Forward);
            v = v.getView (i);
        }

        Shape alloc = EditorPanePrinter.getAllocation (root,
                                                       sourcePane);
    ...
    public static Shape getPageShape (int         pageStartY,
                                      int         pageWidth,
                                      int         pageHeight,
                                      JEditorPane sourcePane)
    {
        Area      result = new Area (new Rectangle (0,
                                                    0,
                                                    pageWidth,
                                                    pageHeight));
               ...
               throws PrinterException
    {
        ...
        {
            pageFormat.getPaper ().setImageableArea (0,
                                                     0,
                                                     paper.getWidth (),
                                                     paper.getHeight ());
